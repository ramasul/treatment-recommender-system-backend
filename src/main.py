...
def create_source_node_graph_web_url(graph, model, source_url, source_type):
    ...
    if pages == None or len(pages) == 0:
        failed_count += 1
        message = f"Unable to load the web page: {source_url}"
...
async def processing_source(uri, userName, password, database, model, file_name, pages, allowedNodes, allowedRelationship, is_uploaded_from_local=None, merged_file_path=None, retry_condition=None, additional_instructions=None):
    """
    [ENG]: Extracts a Neo4jGraph from a PDF file based on the model.
    [IDN]: Mengekstrak Neo4jGraph dari file PDF berdasarkan model.
...
async def processing_chunks(chunkId_chunkDoc_list,graph,uri, userName, password, database,file_name,model,allowedNodes,allowedRelationship, node_count, rel_count, additional_instructions=None):
    ...
    merge_relationship_between_chunk_and_entities(graph, chunks_and_graphDocuments_list)
...
def get_source_list_from_graph(uri,userName,password,db_name=None):
    """
    [ENG]: Get existing files list from graph
    [IDN]: Dapatkan daftar file yang ada dari graf
    Args:
        uri: URI of the graph to extract
...
def update_graph(graph):
    """
    [ENG] Update the graph node with SIMILAR relationship where embedding scrore match
    [IDN] Perbarui node graf dengan hubungan SIMILAR di mana skor embedding cocok
    """
    graph_DB_dataAccess = graphDBdataAccess(graph)
...
def connection_check_and_get_vector_dimensions(graph,database):
    """
    [ENG]: Check the connection from Neo4j and get the vector dimensions
    [IDN]: Mengecek koneksi dari Neo4j dan dapatkan dimensi
    Args:
        uri: URI of the graph to extract
        userName: Username to use for graph creation ( if None will use username from config file )
        password: Password to use for graph creation ( if None will use password from config file )
...
def populate_graph_schema_from_text(text, model, is_schema_description_cheked):
    """
    [ENG]: Extracts schema from text and returns labels and relationshipTypes
    [IDN]: Ekstrak skema dari teks dan return label dan relationshipTypes
    _summary_
