...
from src.shared.utils import create_gcs_bucket_folder_name_hashed, delete_uploaded_local_file, load_embedding_model
...
class graphDBdataAccess:
    ...
    def update_KNN_graph(self):
        """
        [ENG]: Update the graph node with SIMILAR relationship where embedding score match
        [IDN]: Memperbarui node graf dengan hubungan SIMILAR di mana skor embedding cocok
        """
    ...
    def drop_create_vector_index(self, isVectorIndexExist):
        """
        [ENG]: Drop and create the vector index when vector index dimesion are different.
        [IDN]: Melakukan Dop dan membuat indeks vektor apabila dimensi indeks vektor berbeda.
        """
        embedding_model = os.getenv('EMBEDDING_MODEL')
